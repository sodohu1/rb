...
def main_menu(role_value: str):
    if role_value == "owner":
        rows = [
            ...
            [("📢 Broadcast", "bc:menu"), ("📊 Statistics", "u:stats")],
...
async def ad_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) == 2:
        ...
        if not await owns_channel(uid, cid):
            await q.answer("⛔ Not your channel.", show_alert=True)
            return
        return await ad_menu(update, context, cid)
    _, action, cid, *rest = parts
    cid = int(cid)
    if not await owns_channel(uid, cid):
        await q.answer("⛔ Not your channel.", show_alert=True)
        return
...
async def flt_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    uid = update.effective_user.id
    parts = q.data.split(":")
    # flt:{cid} — sirf menu open
    if len(parts) == 2:
        cid = int(parts[1])
        if not await owns_channel(uid, cid):
            await q.answer("⛔ Not your channel.", show_alert=True)
            return
        return await flt_menu(update, context, cid)
    # flt:{action}:{cid} or flt:{action}:{cid}:{key}
    action = parts[1]
    cid = int(parts[2])
    if not await owns_channel(uid, cid):
        await q.answer("⛔ Not your channel.", show_alert=True)
...
async def cap_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) == 2:
        ...
        if not await owns_channel(uid, cid):
            await q.answer("⛔ Not your channel.", show_alert=True)
            return
        return await cap_menu(update, context, cid)
    _, action, cid, *rest = parts
    cid = int(cid)
    if not await owns_channel(uid, cid):
        await q.answer("⛔ Not your channel.", show_alert=True)
...
async def host_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if name == "requirements.txt":
        ...
        if pkgs:
            await update.effective_message.reply_text(
                f"📤 {name} saved.\n🧰 Auto-Install: {len(pkgs)} package(s) install ho rahe hain…\n📦 {', '.join(pkgs)}")
            ok_list, fail_list = [], []
            for pkg in pkgs:
                code, out = await pip_install([pkg])
                if code == 0:
            ...
            await add_log("INFO", "hosting", None, None,
                          f"auto-pip {name}: ok={ok_list} fail={fail_list}")
            result = f"✅ Installed: {', '.join(ok_list) or '—'}"
            if fail_list:
                result += f"\n❌ Failed: {', '.join(fail_list)}"
            await update.effective_message.reply_text(result)
            return
        else:
            msg += "\n✅ Koi missing package nahi — seedha Start kar sakte ho."
...
async def consume(update, context) -> bool:
    ...
    if kind == "ad_custom":
        ...
        for u in users:
            try:
                await msg.copy(u["user_id"])  # supports text + media + captions
                sent += 1
            except Exception:
                fail += 1
...
def main():
    ...
    app.add_handler(MessageHandler(filters.Document.ALL, own_document), group=1)
